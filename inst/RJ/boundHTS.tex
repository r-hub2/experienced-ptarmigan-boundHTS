...
\tightlist
\item
  \textbf{Functions for point estimate inputs}

  \begin{itemize}
  \tightlist
  \item
    \texttt{Poisson\_convolution\_density\_point\_parallel}\strut \\
  \item
    \texttt{Beta\_convolution\_density\_point\_parallel}\strut \\
  \item
    \texttt{ZIB\_convolution\_density\_point\_parallel}\strut \\
  \item
    \texttt{ZOIB\_convolution\_density\_point\_parallel}
  \end{itemize}
\item
  \textbf{Functions for sample-based inputs}

  \begin{itemize}
  \tightlist
  \item
    \texttt{Poisson\_convolution\_density\_parallel}\strut \\
  \item
    \texttt{Beta\_convolution\_density\_parallel}\strut \\
  \item
    \texttt{ZIB\_convolution\_density\_parallel}\strut \\
  \item
    \texttt{ZOIB\_convolution\_density\_parallel}
  \end{itemize}
\end{itemize}

Additionally, the package includes the \texttt{tilt\_density()} function, which exponentially tilts a density toward a specified target mean. This function supports both discrete and continuous distributions, providing a flexible tool for adjusting predictive densities.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{figures/sim_node_hierarchy.pdf}}
...
The next step is to generate the aggregated series density using convolution. This is accomplished with the \texttt{Poisson\_convolution\_density\_point\_parallel()} function, which takes the grid of discrete values (\texttt{z\_values}) and the predictive rate estimates of the bottom-level series (\texttt{lambda\_vector}). Once the convolution density is computed, it is adjusted, or ``tilted'', to align with the predictive mean of the top-level series. The \texttt{tilt\_density()} function performs this adjustment, taking as inputs the predictive mean, the grid of discrete values, and the convoluted density.
...
\subsubsection{Coherent hierarchical forecasting}\label{coherent-hierarchical-forecasting-1}
...
The bottom series are rescaled to the interval {[}0, \(w_{m}\){]} using the \texttt{rBeta\_ab()} from \pkg{ExtDist}, where where \(w_{m}\) denotes the aggregation weight for bottom series \(m\). This rescaling facilitates aggregation through convolution. A set of weighted samples is generated for each bottom-level series and stored in a three-dimensional array. The convolution-based density of the aggregated series is then evaluated over a fine grid on {[}0,1{]} using the \texttt{Beta\_convolution\_density\_point\_parallel()} function. This function takes as inputs the evaluation grid, the parameters of the final bottom-level series, the array of weighted samples, and the corresponding aggregation weight. Finally, the resulting density is tilted toward the predictive mean to ensure coherence with the marginal forecast. This is achieved using the \texttt{tilt\_density()} function.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{boundHTS_files/figure-latex/beta-tilted-plot-1.pdf}}
\caption{\label{fig:beta-tilted-plot}The tilted and convoluted density of the top series using Beta distributed bottom series nodes for a given time point. The dark red colour captures the density of the convolution, the blue represents the tilted denisty and the dashed black line indicates the predictive mean of the top series.}
\end{figure}
...
\subsection{Data generating process for Case 1: Hierarchical Poisson distributions.}\label{data-generating-process-for-case-1-hierarchical-poisson-distributions.}
...
\subsection{Data generating process for Case 2: Hierarchical Beta distributions.}\label{data-generating-process-for-case-2-hierarchical-beta-distributions.}
